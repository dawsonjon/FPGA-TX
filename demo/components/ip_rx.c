...
void ip_rx(){
    ...
    payload_words, payload_length, padding_words, ip_to, ip_from, i, protocol;
        ...
        if(ip_to_hi == local_ip_address_hi &&
           ip_to_lo == local_ip_address_lo){
            ...
            if(protocol == 1){//icmp
                ...
                fputc(payload_length, tcp_out);
                ip_from = (ip_from_hi << 16) | ip_from_lo;
                fputc(ip_from, tcp_out);
                ip_to = (ip_to_hi << 16) | ip_to_lo;
                fputc(ip_to, tcp_out);
                for(i=0; i<payload_words; i++) fputc(fgetc(ethernet_in), tcp_out);
